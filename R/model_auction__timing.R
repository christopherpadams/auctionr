...
# From code_sketch
#######################################################
f.bid_function_fast__orig = function(cost, num_bids, mu, alpha, gamma_1p1oa){
...
vf.bid_function_fast__orig = Vectorize(FUN = f.bid_function_fast__orig,vectorize.args = "cost")
vf.w_integrand_z_fast__orig = function(z, w_bid, num_bids, mu, alpha, gamma_1p1oa, param.u){

  b_z = vf.bid_function_fast__orig(cost=z, num_bids=num_bids, mu=mu, alpha=alpha, gamma_1p1oa)
  u_z = w_bid/b_z
...
f.funk__orig = function(data_vec, param.u){
  val = integrate(vf.w_integrand_z_fast__orig, w_bid=data_vec[1],
                  num_bids=data_vec[2], mu=data_vec[3], alpha=data_vec[4],
...
f.ll_parallel__orig = function(par, y, n, h_x, cl){
  params = par
  v.y = y
  v.n = n
  m.h_x = h_x
  ...
  v.f_w = parApply(cl = cl, X=dat, MARGIN=1, FUN=f.funk__orig, param.u=param.u)
  v.f_y = v.f_w/v.h
...
v.y = data$y
v.n = data$n
m.h_x__log = as.matrix(cbind(log(data$x1),log(data$x2)))
...
dfTime = data.frame(matrix(ncol=2, nrow=3))
for (iRow in 1:nrow(dfTime)) {
  t_am = system.time({
    cl = parallel::makeCluster(num_cores)
    clusterExport(cl,varlist=c("vf.bid_function_fast__orig",
                               "vf.w_integrand_z_fast__orig",
                               "f.funk__orig"))
    optim_control = list(maxit = 2000, parscale = c(1, 0.1, 1, 0.1, rep(1, length(x0) - 4)))
    result_am = optim(par = x0, fn = f.ll_parallel__orig, control=optim_control,
                      y=v.y, n=v.n, h_x=m.h_x__log, cl=cl)
    parallel::stopCluster(cl)
  })[[3]]
...
print(paste("runtime orig =", mean(dfTime[,1]) ))
print(paste("runtime auctionmodeling =", mean(dfTime[,2]) ))
...
#'
#'   # Given sdlog
#'   return(list(sdlog=distrib_std_dev, meanlog=(-distrib_std_dev^2*1/2)))
#' }
#'
#' auction__get_distrib_params__weibull <- function(distrib_std_dev) {
#'   # Given std dev and E(X) = 1, calculate scale and shape
...
#'
#' t0_pj = Sys.time()
#' result_pj = model_auction(dat = list(price = v.y, num = v.n,
#'                          x_terms = m.h_x),
#'               winning_bid = 'price',
#'               number_of_bids = 'num',
#'               init_priv_mu = x0[1],
...
#'                            "f.funk"))
#' optim_control = list(maxit = 2000, parscale = c(1, 0.1, 1, 0.1, rep(1, length(x0) - 4)))
#' result_am = optim(par = x0, fn = f.ll_parallel, control=optim_control,
...
#' print(paste("auction_model  |  runtime=", t1_pj-t0_pj))
#' print(paste("code_sketch from Alex  |  runtime=", t1_am-t0_am))
