...
We first diagnose SE calculation with a default set of `hessian()` arguments, `list(eps=1e-4, d=0.0001, r=4)`, and just one MLE estimation per data set. 
...
Similarly, the standard errors were produced in `r round(mean(!is.na(newpar_results[,"se_mu"]))*100)`% of cases. There does not seem to be much gain from using arguments different from the default values. That said, it might be useful to continue investigating performance of other argument values.
