...
auction__deriv_bid_alpha <- function(alpha, mu, gamma_1p1oa, z, n_bids){
  ...
  alpha_bid_4 = pgamma((n - 1)*(gamma_1p1oa/mu*z)^alpha, 1/alpha, lower = FALSE)*gamma(1/alpha)
  alpha_bid_5 = exp((n_bids - 1)*(gamma_1p1oa/mu*z)^alpha)

  hyper_2F2 = 1/(gamma((n - 1)*(gamma_1p1oa/mu*z)^alpha + 1)*gamma((n - 1)*(gamma_1p1oa/mu*z)^alpha))*
    stats::integrate(t^((n - 1)*(gamma_1p1oa/mu*z)^alpha -1)*
                       hyperg_1F1((n - 1)*(gamma_1p1oa/mu*z)^alpha,
                                  (n - 1)*(gamma_1p1oa/mu*z)^alpha + 1, -1/alpha*t),
                     lower = 0, upper = 1, abs.tol = 1e-10)
  ...
  deriv_alpha_bid_4 = (1/alpha)^(1 + (n_bids - 1)*(gamma_1p1oa/mu)^alpha)*exp(-1/alpha)
    + (n_bids - 1)*(gamma_1p1oa.mu*z)^alpha(
      (gamma((n - 1)*(gamma_1p1oa/mu*z)^alpha))^2*(1/alpha)^((n_bids - 1)*(gamma_1p1oa/mu*z)^alpha)*
        hyper_2F2 -
        (gamma((n - 1)*(gamma_1p1oa/mu*z)^alpha) - pgamma((n - 1)*(gamma_1p1oa/mu*z)^alpha, 1/alpha, lower = FALSE)*gamma(1/alpha))*log(1/alpha) +
        gamma((n - 1)*(gamma_1p1oa/mu*z)^alpha)*digamma((n - 1)*(gamma_1p1oa/mu*z)^alpha)
    )*(log(gamma_1p1oa/mu*z) - digamma(1+1/alpha)/alpha)
  deriv_alpha_bid_5 = -exp(-(n - 1)*(gamma_1p1oa/mu*z)^alpha)*(n - 1)*(gamma_1p1oa/mu*z)^alpha*(
    log(gamma_1p1oa/mu*z) - digamma(1+1/alpha)/alpha
  )
...
auction__deriv_alpha_integrand <- function(alpha, w_bid, mu, gamma_1p1oa, z, n_bids, distrib_std_dev, id_distrib){
  ...
  deriv_unobs = auction__deriv_unobs(distrib_std_dev = distrib_std_dev,
                                     ...
                                     w_bid = w_bid,
                                     b_z = b_z)
  deriv_unobs_u = deriv_unobs$deriv_unobs_u
  f_unobs = deriv_unobs$f_unobs
...
auction__deriv_ll_alpha <- function(x0, dat_X, dat__winning_bid, dat__n_bids, cl, listFuncCall){
  ...
  deriv_alpha_log_likelihood = -sum(log(v.f_deriv_y_alpha))
...
auction__deriv_bid_mu <- function(z, alpha, mu, gamma_1p1oa, n_bids){
  ...
  mu_bid_2 = pgamma((n - 1)*(gamma_1p1oa/mu*z)^alpha, 1/alpha, lower = FALSE)*gamma(1/alpha)
  mu_bid_3 = exp((n_bids - 1)*(gamma_1p1oa/mu*z)^alpha)

  deriv_mu_bid_1 = 1/(alpha*gamma_1p1oa)*(n_bids-1)^(-1/alpha)
  deriv_mu_bid_2 = -1/mu*alpha*(n_bids-1)*(gamma_1p1oa/mu*z)^alpha*(
    (gamma((n - 1)*(gamma_1p1oa/mu*z)^alpha))^2*(1/alpha)^((n_bids - 1)*(gamma_1p1oa/mu*z)^alpha)*
      hyper_2F2 -
      (gamma((n - 1)*(gamma_1p1oa/mu*z)^alpha) - pgamma((n - 1)*(gamma_1p1oa/mu*z)^alpha, 1/alpha, lower = FALSE)*gamma(1/alpha))*log(1/alpha) +
      gamma((n - 1)*(gamma_1p1oa/mu*z)^alpha)*digamma((n - 1)*(gamma_1p1oa/mu*z)^alpha)
  )
  deriv_mu_bid_3 = -exp((n_bids - 1)*(gamma_1p1oa/mu*z)^alpha)*(n-1)*(gamma_1p1oa*z)^alpha*alpha*mu^(-alpha-1)
...
auction__deriv_ll_mu <- function(x0, dat__winning_bid, dat__n_bids, dat_X, cl, listFuncCall){
  ...
  deriv_mu_log_likelihood = -sum(log(v.f_deriv_y_mu))
  return(list(deriv_mu_log_likelihood,v.f_deriv_y_mu))
}

######
#sigma^2
auction__deriv_sig_integrand <- function(distrib_std_dev, w_bid, id_distrib, z, n_bids, mu, alpha, gamma1p1oa){
  b_z = vf__bid_function_fast(cost=z, n_bids=n_bids, mu=mu, alpha=alpha, gamma_1p1oa)
  deriv_unobs = auction__deriv_unobs(distrib_std_dev = distrib_std_dev,
                                     id_distrib = id_distrib,
                                     w_bid = w_bid,
                                     b_z = b_z)
  deriv_unobs_sig = deriv_unobs$deriv_unobs_sig
  f_unobs = deriv_unobs$f_unobs
...
auction__deriv_ll_sig <- function(x0, dat__winning_bid, dat__n_bids, dat_X, cl){
  listIdx = auction__x0_indices()
  ...
  deriv_sig_log_likelihood = -sum(log(v.f_deriv_y_sig))
  return(list(deriv_sig_log_likelihood, v.f_deriv_y_sig))
}

#######
#standard errors
auction__se <- function(x0, dat__winning_bid, dat__n_bids, dat_X, cl){
  ...
  auction__deriv_ll_alpha = auction__deriv_ll_alpha(x0 = x0,
                                                    ...
                                                    cl = cl,
                                                    listFuncCall = listFuncCall)
  v.f_deriv_y_alpha = auction__deriv_ll_alpha$v.f_deriv_y_alpha
  ...
  auction__deriv_ll_mu = auction__deriv_ll_mu(x0 = x0,
                                              ...
                                              cl = cl,
                                              listFuncCall = listFuncCall)
  v.f_deriv_y_mu = auction__deriv_ll_alpha$v.f_deriv_y_mu
  ...
  auction__deriv_ll_sig = auction__deriv_ll_sig(x0 = x0,
                                                ...
                                                cl = cl,
                                                listFuncCall = listFuncCall)
  v.f_deriv_y_sig = auction__deriv_ll_alpha$v.f_deriv_y_sig


all_prod <- list()
for (i in 1:obs){
  score_[i] = c(v.f_deriv_y_alpha, v.f_deriv_y_mu, v.f_deriv_y_sig)
  prod[i] = score[i] %*% t(score[i])
  all_prod[[i]] <- prod[i]
}
...
x0[listIdx$pv_weibull_mu] = def_pv_mu
...
listFuncCall = list(funcName = "dgamma",
                    funcID = auction__get_id_distrib(sFuncName = "dgamma"))
